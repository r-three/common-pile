...
def main(input_glob, output_dir, slice_idx, num_slices, device="cuda:0", num_workers=5, **kwargs):
    os.makedirs(output_dir, exist_ok=True)
    files_to_convert = filter(lambda f: get_slice(os.path.basename(f), num_slices) == slice_idx and is_pdf(f) and not output_exists(f, output_dir), glob.glob(input_glob))
    ...
    num_errors = 0
    with mp.Pool(processes=num_workers, initializer=worker_init, initargs=(model_dict,), maxtasksperchild=10) as pool:
        pbar = tqdm(desc="Processing PDFs", unit=" files")
        for markdown, fpath in pool.imap_unordered(process_single_pdf, files_to_convert):
            if markdown is None:
                pbar.update(1)
                num_errors += 1
                continue
            ...
            pbar.set_postfix({"Errors": num_errors, "MB Written": f"{total_bytes_written/1e6:.3E}"})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-glob", required=True, help="Directory containing PDF files")
    parser.add_argument("--output-directory", required=True, help="Directory to save markdown files")
    parser.add_argument("--device", default="cuda:0", help="Device to run the marker model on (e.g., 'cuda:0' or 'cpu')")
    parser.add_argument("--num-workers", type=int, default=5)
    parser.add_argument("--slice-idx", type=int, default=0)
    parser.add_argument("--num-slices", type=int, default=5)
    args = parser.parse_args()

    main(args.input_glob, args.output_directory, args.slice_idx, args.num_slices, device=args.device, num_workers=args.num_workers)
