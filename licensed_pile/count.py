...
class TokenCountParallel(BaseParallelProcessor):
    @classmethod
    def increment_progressbar(
        cls, queue: Queue, /, shards: int = 0, documents: int = 0, tokens: int = 0
    ):
        return super().increment_progressbar(
            queue, shards=shards, documents=documents, tokens=tokens
        )
        ...
        **kwargs,
        ...
        with smart_open.open(source_path) as f:
            ...
            try:
                for i, line in enumerate(f):
                    ...
                    if document_count % update_interval == 0:
                        cls.increment_progressbar(
                            queue, documents=document_count, tokens=token_count
                        )
                ...
                return
            cls.increment_progressbar(
                queue, shards=1, documents=document_count, tokens=token_count
            )


def main():
    mp.set_start_method("spawn")
    parser = argparse.ArgumentParser(description="Count tokens in dolma files.")
    parser.add_argument(
        "--input",
    ...
    args.input = re.sub("documents/?$", "", args.input)
    with TemporaryDirectory() as tempdir:
        processor = TokenCountParallel(
            source_prefix=os.path.join(args.input, "**", "*.jsonl.gz"),
            # Unused
            destination_prefix=args.input,
