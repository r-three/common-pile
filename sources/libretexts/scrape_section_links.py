...
def get_section_carets():
    ...
    try:
        ...
        carets = driver.find_elements(
            By.XPATH, "//i[contains(@class, 'caret right icon cursor-pointer')]"
        )
        ...
        carets = [
            caret
            for caret in carets
            if caret.find_element(
                By.XPATH, "./following-sibling::div[@class='content']//a"
            ).text.strip()
            not in ["Front Matter", "Back Matter"]
        ]
        return carets
...
def click_all_carets(carets):
    ...
    for caret in tqdm(carets, desc="Expanding Sections", unit=" sections", leave=False):
        try:
            section_title_element = caret.find_element(
                By.XPATH, "./following-sibling::div[@class='content']//a"
            )
...
def get_all_subsection_links():
    ...
    try:
        ...
        subsection_items = driver.find_elements(
            By.XPATH,
            "//div[@class='item' and @role='listitem'][.//i[contains(@class, 'circle tiny icon')]]",
        )

        for item in tqdm(
            subsection_items,
            desc="Extracting Subsections",
            unit=" subsections",
            leave=False,
        ):
            try:
...
with open("libretext_content_urls.csv", "w") as f:
    ...
    for book_url in pbar:
        ...
        writer.writerows(
            [
                [book_url.strip(), section_name, section_url.strip()]
                for section_name, section_url in subsections.items()
            ]
        )
