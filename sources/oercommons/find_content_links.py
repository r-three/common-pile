...
with open("oercommons_search_results.csv", "r") as f_in, open(
    "oercommons_links.csv", "w"
) as f_out:
    ...
    for row in pbar:
        try:
            ...
            if re.match(
                r"https://oercommons.org/courseware/lesson/*", search_result_link
            ):
                response = requests.get(search_result_link, headers=HEADERS)
                soup = BeautifulSoup(response.text, "html.parser")
                content_link = soup.find_all(
                    "a", href=True, text=re.compile("View Resource")
                )[0]["href"]
                writer.writerow([row[0], row[1], content_link, row[2]])
                num_links += 1
                pbar.set_postfix({"num_links": num_links})
            elif re.match(
                r"https://oercommons.org/courseware/unit/*", search_result_link
            ):
                response = requests.get(search_result_link, headers=HEADERS)
                soup = BeautifulSoup(response.text, "html.parser")
                ...
                content_links = [
                    ...
                    for l in all_links
                    if re.match(
                        r"https://oercommons.org/courseware/lesson/*", l["href"]
                    )
                ]
