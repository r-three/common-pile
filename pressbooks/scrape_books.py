...
with open("pressbooks_links.csv", "r") as f_in, open("pressbooks_content.csv", "w") as f_out:
    reader = csv.reader(f_in)
    header = next(reader)

    writer = csv.writer(f_out)
    writer.writerow(["title", "author", "subject", "institution", "language", "license", "last_updated", "book_url", "page_url", "page_content"])
    
    num_sections = 0
    pbar = tqdm(list(reader), unit=" books")
    for row in pbar:
        title, author, subject, institution, language, license, last_updated, book_url = row
        try:
        ...
        for link in tqdm(links, unit=" sections", leave=False):
            ...
            writer.writerow([title, author, subject, institution, language, license, last_updated, book_url, link, content])
            num_sections += 1
